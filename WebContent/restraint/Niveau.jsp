...
<%
    List<Niveau> niveaus ;
    Niveau selected = null;
    NiveauDao met = new NiveauDao();
	
    niveaus= met.allNiveau();
	if(request.getParameter("id")!=null){
		for(Niveau annn : niveaus){
   ...
   <div class="container">
     ...
     <!-- form ajouter -->
       <div class="col">
         ...
         <hr>
				<form action="<%=request.getContextPath()%>/niveau" method="POST">
       ...
       <div class="coll">
        ...
        <!--  form recherche -->
        <div class="fr">
         <form class="form-inline" action="<%=request.getContextPath()%>/niveau" method="POST">
			<div class="form-group">
	            ...
	            <table class="table table-bordered table-hover">
	              <tr>
	                 ...
	                 <td>Action</td>
	                    ...
	                    <%
	                      Niveau yess=(Niveau)request.getAttribute("seach");
			              int count = 0;
					      NiveauDao metier=new NiveauDao();
			              List<Niveau> ans=metier.allNiveau();
			              for(Niveau annee : ans){
	                     %>
	                  <tr class="<%=count%2==0?"":"success" %>">
	                   <td><%=annee.getId() %></td>
	                   <td><%=annee.getDescription()%></td>
	                   <td><%=annee.getCodeNiveau()%></td>
         ...
         <div class="modal-content">
            <div class="modal-header">
               ...
               <h4 class="modal-title" id="header">
                <span id="titre">Modifier une vacation</span>
               </h4>
            </div>
            <div class="modal-body">
				<form class="form-horizontal form-label-left"
						action="<%=request.getContextPath()%>/niveau"
...
<input type="hidden" id="id" name="id" value="<%=request.getParameter("id")!=null?request.getParameter("id"):"" %>" />
						...
						<div class="form-group col-md-12 col-sm-12 col-xs-12">
							...
							<input type="text" name="codem" id="codem" value="<%=selected!=null?selected.getCodeNiveau():"" %>" required="required" class="form-control">
						</div>
		...
		$(document).ready(function () {
			if($("#id").val()!==''){
				if($("#operation").val()==="UPDATE"){
					...
					$("#titre").text("Supprimer un niveau");
				}
