...
<body>
<%
    List<TypeAcademique> typeAcademiques ;
    TypeAcademique selected = null;
    TypeAnneeAcademiqueDao met = new TypeAnneeAcademiqueDao();
	
    typeAcademiques= met.allTypeAcademique();
	if(request.getParameter("id")!=null){
		for(TypeAcademique annn : typeAcademiques){
...
<!-- fin du bar de navigation -->  
 ...
 <!-- hhkl -->
   ...
   <div class="container">
     ...
     <!-- form ajouter -->
       <div class="col">
         <h1>Ajouter un type d'annee academique</h1>
         <hr>
				<form action="<%=request.getContextPath()%>/type" method="POST">
					  <div class="form-group">
					    <label for="description">Description</label>
					    <input type="text" class="form-control" id="description" placeholder="Enter description" name="desc" required>
					  </div>
					  <div class="form-group">
       ...
       <div class="coll">
         <div class="con">
            <p>
             Liste des types d'annee academique
            </p>
         </div>
         <div class="clear"></div>
         <br>
        ...
        <!--  form recherche -->
        <div class="fr">
         <form class="form-inline" action="<%=request.getContextPath()%>/type" method="POST">
			<div class="form-group">
	            ...
	            <table class="table table-bordered table-hover">
	              <tr>
	                 ...
	                 <td>Action</td>
	                    ...
	                    <%
	                      TypeAcademique yess=(TypeAcademique)request.getAttribute("seach");
			              int count = 0;
					      TypeAnneeAcademiqueDao metier=new TypeAnneeAcademiqueDao();
			              List<TypeAcademique> ans=metier.allTypeAcademique();
			              for(TypeAcademique annee : ans){
	                     %>
	                  <tr class="<%=count%2==0?"":"success" %>">
	                   <td><%=annee.getId() %></td>
	                   <td><%=annee.getDescription()%></td>
	                   <td><%=annee.getCodeTypeAcademique() %></td>
         ...
         <div class="modal-content">
            <div class="modal-header">
               ...
               <h4 class="modal-title" id="header">
                <span id="titre">Modifier un type d'annee academique</span>
               </h4>
            </div>
            <div class="modal-body">
				<form class="form-horizontal form-label-left"
						action="<%=request.getContextPath()%>/type"
...
<input type="hidden" id="id" name="id" value="<%=request.getParameter("id")!=null?request.getParameter("id"):"" %>" />
						...
						<div class="form-group col-md-12 col-sm-12 col-xs-12">
							...
							<input type="text" name="codem" id="codem" value="<%=selected!=null?selected.getCodeTypeAcademique():"" %>" required="required" class="form-control">
						</div>
		...
		$(document).ready(function () {
			if($("#id").val()!==''){
				if($("#operation").val()==="UPDATE"){
					...
					$("#titre").text("Supprimer un type d'annee academique");
				}
