   ...
   <div class="container">
     ...
     <!-- form ajouter -->
       <div class="col">
         ...
         <hr>
				<form action="<%=request.getContextPath()%>/option" method="POST">
       ...
       <div class="coll">
        ...
        <div class="fr">
         <form class="form-inline" action="Recherche.jsp" method="POST">
		    ...
		    <input type="hidden" name="operation" value="FINDOP" />
		 </form>
         </div>
         <!-- fin form recherche -->
	            ...
	            <table class="table table-bordered table-hover">
	              <tr>
	                 ...
	                 <td>Action</td>
	                    ...
	                    <%
	                     if(request.getAttribute("optseach")==null){
	                      Option yess=(Option)request.getAttribute("seach");
			              int count = 0;
					      OptionDao metier=new OptionDao();
			              List<Option> ans=metier.allOption();
			              for(Option annee : ans){
	                     %>
	                  <tr class="<%=count%2==0?"":"success" %>">
	                   <td><%=annee.getId() %></td>
	                   <td><%=annee.getDescription()%></td>
	                   <td><%=annee.getCode_option() %></td>
	                  ...
	                  %>
	                  <%}else{
	                	  Option yess=(Option)request.getAttribute("optseach");
	                	  %>
		                  <tr>
		                   <td><%=yess.getId() %></td>
		                   <td><%=yess.getDescription()%></td>
		                   <td><%=yess.getCode_option()%></td>
		                   <td><a href="Option.jsp?id=<%=yess.getId()%>&operation=UPDATE"
														class="btn btn-info btn-xs" data-toggle="modal">
														<i class="fa fa-pencil"></i> Modifier </a> 
							   <a href="Option.jsp?id=<%=yess.getId()%>&operation=REMOVE" 
														class="btn btn-danger btn-xs " data-toggle="modal">
														<i class="fa fa-trash-o"></i>Supprimer </a>
							</td>
		                   </tr>
		                  <%
		                  request.setAttribute("optseach",null);
	                  } %>		
	            </table>
	          </div>
        </div>
         ...
         <div class="modal-content">
            <div class="modal-header">
               ...
               <h4 class="modal-title" id="header">
                <span id="titre">Modifier une option</span>
               </h4>
            </div>
            <div class="modal-body">
				<form class="form-horizontal form-label-left"
						action="<%=request.getContextPath()%>/option"
...
<input type="hidden" id="id" name="id" value="<%=request.getParameter("id")!=null?request.getParameter("id"):"" %>" />
						...
						<div class="form-group col-md-12 col-sm-12 col-xs-12">
							...
							<input type="text" name="codem" id="codem" value="<%=selected!=null?selected.getCode_option():"" %>" required="required" class="form-control">
						</div>
		...
		$(document).ready(function () {
			if($("#id").val()!==''){
				if($("#operation").val()==="UPDATE"){
					...
					$("#titre").text("Supprimer une option");
				}
